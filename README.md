...
**Current proof-pack:** The production-shaped semantic-cache proof path is now implemented as a local harness: LatticeMemory proxy + PQ cache + Redis shared store + flywheel review, measured against exact string, dense semantic-cache, RedisVL direct, and GPTCache-style baselines with hit rate, latency, savings, Redis memory, review behavior, and false-positive rate reported together. The strongest current artifact uses the public Bitext customer-support dataset and shows why PQ should be treated as a candidate generator with validation before serving: raw PQ over-hits, while `lattice_pq_redis_validated_cosine` keeps a 0.9812 hit rate with zero measured false positives on the 6,000-request Redis Stack split. A live proxy replay with Redis-backed cache cosine validation reaches a 0.9917 hit rate with zero measured false positives and a 0.0083 upstream-call rate. See [`docs/proxy_pq_redis_flywheel_proof_pack_2026-07-03.md`](docs/proxy_pq_redis_flywheel_proof_pack_2026-07-03.md). The public claim remains scoped: this is evidence for repeated/paraphrased support-workload caching, not a general vector database or RAG replacement claim.
